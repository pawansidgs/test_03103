...
var environment = config['test_03103'].env;
...
console.log('&test_03103 api: [' +  domains[environment] + ', ' + config['test_03103'].basepath + ']');

BeforeAll("BeforeFeatures", function(next) {
    if (apps[0].name) {
        console.log ("Getting Creds from DevAppsKeys.json");
        getCreds(config['test_03103'].app, config['test_03103'].product);
    } else
    {
...
Before(function(scenario, callback) {
    ...
    this.apickli = new apickli.Apickli(tokens[0],
        tokens[1]+ config['test_03103'].basepath,
        './test/integration/features/fixtures/');
    callback();
});
